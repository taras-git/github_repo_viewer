...
class SearchHistoryRepository {
    ...
    return _store
        .query(
            finder: filter != null && filter.isNotEmpty
                ...
                : null)
        .onSnapshots(_sembastDatabase.dbInstance)
        .map((records) => records.reversed.map((r) => r.value).toList());
  }

  Future<void> addSearchTerm(String term) =>
      _addSearchTerm(term, _sembastDatabase.dbInstance);
  ...
  Future<void> deleteSearchTerm(String term) =>
      _deleteSearchTerm(term, _sembastDatabase.dbInstance);
  ...
  Future<void> putSearchTermFirst(String term) async {
    await _sembastDatabase.dbInstance.transaction((trx) async {
